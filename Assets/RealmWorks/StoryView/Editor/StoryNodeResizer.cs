...
{
    public class StoryNodeResizer : VisualElement
    {
        ...
        public StoryNodeResizer() :
            this(new Vector2(30.0f, 30.0f))
        {
        }

        public StoryNodeResizer(Vector2 minimumSize)
        {
...
//                return;
        ...
        {
            var storyNode = parent as StoryNode;
            if (storyNode == null)
                return;
//
//            if (!ce.IsResizable())
...
//                newSize.y = (newSize.y < minHeight) ? minHeight : ((newSize.y > maxHeight) ? maxHeight : newSize.y);
                ...
                if (storyNode.GetPosition().size != newSize)
                {
//                    if (parent.style.positionType == PositionType.Manual)
                    {
                        
                        //ce.layout = new Rect(ce.layout.x, ce.layout.y, newSize.x, newSize.y);
                        storyNode.style.height = newSize.y;
                        storyNode.style.width = newSize.x;
                        
//                        storyNode.ResizeTo(newSize);
...
//                if (resized)
//                {
//                    storyNode.UpdatePresenterPosition();
//
//                    GraphView graphView = storyNode.GetFirstAncestorOfType<GraphView>();
//                    if (graphView != null && graphView.elementResized != null)
//                    {
//                        graphView.elementResized(storyNode);
//                    }
//                }
//
//                e.StopPropagation();
